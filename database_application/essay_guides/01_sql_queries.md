# SQL & 查詢題目完全解析 - 申論題答題框架

這份文件針對**所有 SQL & 查詢相關題目**提供詳細的申論題答題架構，依照考試標準格式讓媽媽可以有系統地準備考試。
...
| 題型 | 占比 | 代表題目 |
|------|------|---------|
| **SQL語法實作** | 45% | SELECT、JOIN、GROUP BY、聚合函數 |
| **View/Trigger/Stored Procedure概念** | 20% | 定義、優缺點、使用時機 |
| **資料庫設計+SQL** | 25% | ER Diagram → 資料表 → SQL查詢 |
| **進階查詢技巧** | 10% | 子查詢、關聯代數、複雜JOIN |
...
```sql
-- 題目：找出學生姓名與其曾經修過某位老師剛好兩門課的老師姓名

-- 資料表：
-- STUDENT(S#, Sname, Saddress, D#)
-- TEACHER(T#, Tname, Taddress, D#)
-- TAKE_COURSE(S#, C#, Score)
-- OFFER(T#, C#, Classroom)

-- 標準答案：
SELECT DISTINCT S.Sname AS 學生姓名, T.Tname AS 老師姓名
FROM STUDENT S
JOIN TAKE_COURSE TC ON S.S# = TC.S#
JOIN OFFER O ON TC.C# = O.C#
JOIN TEACHER T ON O.T# = T.T#
GROUP BY S.S#, S.Sname, T.T#, T.Tname
HAVING COUNT(DISTINCT TC.C#) = 2;
```

**解題關鍵**：

1. 需要連接4個表
2. 使用 `GROUP BY` + `HAVING COUNT() = 2`
3. 注意 `DISTINCT` 的使用（避免重複計算）

---

### 【題型五】聚合函數進階 - GROUP BY + HAVING
...
```sql
-- 題目：至少兩個人的部門，取出部門名稱及年齡大於25歲的人數

-- 資料表：
-- EMPLOYEE(ssn, age, dno)
-- DEPARTMENT(dnum, dname)

-- 標準答案：
SELECT 
    D.dname AS 部門名稱,
    COUNT(CASE WHEN E.age > 25 THEN 1 END) AS 超過25歲人數
FROM 
    DEPARTMENT D
...
1. `HAVING COUNT() >= 2` 篩選「至少兩個人的部門」
2. 使用 `CASE WHEN` 條件式計數
3. 或使用子查詢先篩選部門再JOIN
...
```sql
-- 題目：查出從來沒有搭乘過"甲航空"的旅客

-- 資料表：
-- 旅客(護照號碼, 姓名, 電話, 年齡)
-- 航班(航班編號, 航空公司, 出發時間, 抵達時間)
...
FROM 旅客
WHERE 護照號碼 NOT IN (
    SELECT DISTINCT P.護照號碼
    FROM 購票資訊 P
    INNER JOIN 航班 F ON P.航班編號 = F.航班編號
    WHERE F.航空公司 = '甲航空'
);
...
1. 「沒有」→ 使用 `NOT IN` 或 `NOT EXISTS`
2. `NOT EXISTS` 通常效能較佳
3. 注意 NULL 值問題（NOT IN 遇到 NULL 會有問題）
...
| 題型 | 閱讀分析 | 撰寫SQL | 檢查 | 總計 |
|-----|---------|--------|------|-----|
| 簡單查詢 | 2分鐘 | 5分鐘 | 1分鐘 | 8分鐘 |
| 多表JOIN | 3分鐘 | 8分鐘 | 2分鐘 | 13分鐘 |
| 複雜子查詢 | 4分鐘 | 10分鐘 | 3分鐘 | 17分鐘 |
| 概念題 | 5分鐘 | 12分鐘 | 3分鐘 | 20分鐘 |
...
- [ ] JOIN類型正確（INNER/LEFT/RIGHT）
- [ ] 子查詢回傳值類型正確（單值/多值/多欄）
- [ ] NULL值處理（COALESCE/IFNULL）
- [ ] 聚合函數位置正確
- [ ] DISTINCT使用合理
...
1. **114年高考第三題** - 多表JOIN + 聚合 + 欠款計算
2. **106年地特第二題** - 複雜WHERE條件 + 自我JOIN
3. **112年地特第二、三題** - 關聯代數 + SQL + 子查詢
4. **113年警察第四、五題** - Trigger/SP + View 概念
5. **111年高考第一題** - 員工產品研發 - 典型JOIN題

### 進階題目（★★ 中頻）

6. **109年關務第二題** - 旅行資料庫 + 複雜條件
7. **110年國安第一題** - 部門員工眷屬 + 多層JOIN
8. **107年警察第三題** - 水蜜桃訂購 + GROUP BY
9. **108年司法第一、二題** - 資料完整性 + 權限設計

### 觀念加強（★ 低頻但重要）

10. **106年警察第三題** - View 深入概念
11. **109年地特第五題** - 關聯代數完整性
12. **105年關務第一題** - ODBC/Client-Server架構
...
- [ ] 字串函數：CONCAT, SUBSTRING, UPPER, LOWER
- [ ] 日期函數：YEAR, MONTH, DAY, NOW, DATEDIFF
...
- [ ] Index（索引）原理
- [ ] Transaction（交易）與SQL的關係

---

媽媽加油！這份解析涵蓋了所有SQL重要題型，按照這個架構練習，考試一定沒問題 💪
