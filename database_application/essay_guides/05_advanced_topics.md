# 05_advanced_topics.md - 進階主題申論題完全解析

## 📊 題目總覽與統計

**進階主題 (Advanced Topics)** 涵蓋了 NoSQL、大數據、資料倉儲與分散式資料庫。隨著技術演進，這類題目的比重逐年上升，特別是 NoSQL 與傳統關聯式資料庫的比較。

- **總題數**：約 61 題（佔比約 25-30%）
- **出題頻率**：⭐⭐⭐⭐（近年熱門考點）
- **難度**：⭐⭐⭐（觀念重於實作，需廣泛涉獵）
- **主要題型**：
    1. **NoSQL**：SQL vs NoSQL 比較、CAP 定理、JSON 資料模型設計。
    2. **大數據生態系**：資料倉儲 (Data Warehouse)、資料探勘 (Data Mining)、OLAP vs OLTP。
    3. **分散式資料庫**：分散式 vs 集中式、兩階段提交 (2PC)、資料複製與切割。
...
1. **CAP 定理 (CAP Theorem)**
    - 在分散式系統中，**Consistency (一致性)**、**Availability (可用性)**、**Partition Tolerance (分區容錯性)** 三者不可兼得，最多只能滿足兩項 (通常是 AP 或 CP)。

2. **分散式資料庫透明性 (Transparency)**
    - **位置透明性**：使用者不必知道資料存放在哪個站點。
    - **複製透明性**：使用者不必知道資料有幾份複本。
    - **分割透明性**：使用者不必知道資料表是如何被切割 (水平/垂直) 的。

3. **兩階段提交 (Two-Phase Commit, 2PC)**
    - 確保分散式交易原子性的協定。
    - **Phase 1 (Prepare)**：協調者詢問所有參與者是否準備好提交。
    - **Phase 2 (Commit/Abort)**：若所有人都回覆 OK，則正式提交；否則全體取消。
...
- 寫到 **NoSQL** 必提：**「CAP 定理」、「BASE」、「Schema-less」、「水平擴充」**。
- 寫到 **Data Warehouse** 必提：**「Inmon 的四大特性 (主題/整合/非揮發/時間)」、「ETL」**。
- 寫到 **Distributed DB** 必提：**「透明性 (Transparency)」、「可靠性」、「2PC」**。

### 🎨 圖解加分

- **大數據架構圖**：畫出從 OLTP 到 Data Warehouse 到 OLAP 的流向，非常加分。
- **JSON 範例**：遇到 NoSQL 資料模型題，直接寫一段 JSON 代碼比文字描述更清楚。
...
1. **113 調查局三等** (SQL vs NoSQL)：六大面向比較。
2. **108 關務三等** (Big Data)：解釋 OLTP/OLAP/DW/ETL 關係。

### ★★ 進階 (實作)

1. **109 外交四等** (JSON)：關聯式轉 JSON 實作。
2. **113 一般警察** (Distributed)：分散式優缺點比較。

---

> **💡 媽媽的備忘錄**：
>
> - **SQL** 就像是「銀行帳本」，一筆一筆都要對得清清楚楚，絕對不能錯 (ACID)。
> - **NoSQL** 就像是「便利貼牆」，隨便你貼什麼、貼哪裡都可以，重點是大家都能看得到、貼得快 (BASE)。
> - **Data Warehouse** 就像是「圖書館的過期報紙區」，把每天的新聞 (OLTP) 整理好放進去，讓以後的人可以查閱歷史趨勢 (OLAP)，但你不會去改以前的報紙內容 (Non-volatile)。
