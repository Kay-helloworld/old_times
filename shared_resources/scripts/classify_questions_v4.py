...
def extract_exam_type(file_content):
    """Extract precise exam type from the first few lines of content."""
    ...
    for i, line in enumerate(lines[:5]):
        ...
        # Pattern matching for exam types
        if "身心障礙人員考試" in line:
            return "身心障礙特考"
        elif "關務人員" in line:
            return "關務特考"
        elif "警察人員" in line or "警察特考" in line:
            return "警察特考"
        elif "調查人員" in line:
            return "調查特考"
        elif "國家安全情報人員" in line:
            return "國安特考"
        elif "地方政府公務人員" in line or "地方特考" in line:
            # Check for level - could be 三等 or 四等
            for check_line in lines[:10]:
                if "三等" in check_line and "等" in check_line:
                    return "地特三等"
                elif "四等" in check_line and "等" in check_line:
                    return "地特四等"
            return "地方特考"
        elif "高等考試三級" in line or ("高等考試" in line and "三級" in line):
            return "高考三級"
        elif "高等考試二級" in line or ("高等考試" in line and "二級" in line):
            return "高考二級"
        elif "高等考試一級" in line or ("高等考試" in line and "一級" in line):
            return "高考一級"
        elif "普通考試" in line:
            return "普通考試"
    
    # Fallback - check for "等別" line
...
def get_exam_info(filename, file_content):
    ...
    year = filename[:3]
    exam_type = extract_exam_type(file_content)
...
def parse_questions_from_file(filename, content):
    ...
    year, exam_type = get_exam_info(filename, content)
...
def main():
    ...
    for cat_id, questions in category_buckets.items():
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            for q in questions:
                title = f"## [{q['year']}] [{q['exam_type']}] {q['q_header']}{q['filename']}"
                f.write(f"{title}\n")
    ...
    if unclassified:
        with open(os.path.join(output_dir, "unclassified.md"), 'w', encoding='utf-8') as f:
            ...
            for q in unclassified:
                f.write(f"## [{q['year']}] [{q['exam_type']}] {q['filename']}\n")
                f.write(f"{q['text']}\n\n")
                f.write("---\n\n")
