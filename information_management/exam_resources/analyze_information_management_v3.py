...
2. 將所有資安類別合併為一個大類「資訊安全」
3. 根據題目內容關鍵字分類，而非考卷名稱
4. 按三等+近三年數量排序
"""
...
"""
    ...
    for rank, category in enumerate(sorted_categories, 1):
        ...
        # 資訊安全類別特別標註包含的子領域
        if category == "資訊安全":
            report += "**包含子領域**: 密碼學基礎、網路安全與網路基礎、系統與軟體安全、資安管理制度、營運持續與復原、資安法令與規範、新興技術安全\n\n"
...
- **資訊安全**: 合併所有資安相關關鍵字（密碼學、網路安全、系統安全、資安管理、營運持續、資安法令、新興技術安全）

## 📌 版本更新說明 (v3)

**改進項目**:
1. ✅ 移除過於廣泛的關鍵字（資訊系統、資訊管理、MIS等）
2. ✅ **合併所有資安類別為一個大類「資訊安全」**（重要）
3. ✅ 根據題目內容關鍵字分類，而非考卷名稱
4. ✅ 按「三等+近三年」數量排序，並加入排名編號
...
def main():
    ...
    # 儲存報告
    output_file = output_dir / 'information_management_analysis_v3.md'
    ...
    # 生成關鍵字列表
    keywords_file = output_dir / 'all_keywords_v3.txt'
    with open(keywords_file, 'w', encoding='utf-8') as f:
        f.write("# 資訊管理科目 - 關鍵字列表 v3（按三等+近三年重要性排序）\n\n")
        f.write("## 說明\n\n")
        f.write("所有資安相關關鍵字已合併為一個大類「資訊安全」\n\n")
        ...
        for rank, category in enumerate(sorted_categories, 1):
            ...
            if category == "資訊安全":
                f.write("**包含**: 密碼學、網路安全、系統安全、資安管理、營運持續、資安法令、新興技術安全\n\n")
